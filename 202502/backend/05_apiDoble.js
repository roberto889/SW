...
miAPI.get('/paises', (req, res) => urbano.damePaisesFiltro('Europe')
    .then (paises => {
...
// metodo get para lectura de continentes
miAPI.get('/continentes', (req, res) => urbano.dameContinentes()
    .then (continentes => {
            console.log('Piden lista de continentes y devuelvo', continentes.length,'elementos');
            ponerCabeceraCORS(req, res, false);
            res.json(continentes);
        }
    ));
// Metodo GET: para lectura de ciudades
miAPI.get('/paises/:continente', (req, res) => urbano.damePaisesFiltro(req.params.continente)
    .then (paises => {
            console.log('Piden lista de paises en %s y devuelvo %d elementos', req.params.continente, paises.length);
            ponerCabeceraCORS(req, res, false);
            res.json(paises);
        }
    ));
...
miAPI.get('/civitas', (req, res) => urbano.dameCiudades()
    ...
    // Metodo GET: para lectura de ciudades
    miAPI.get('/filtro', (req, res) => urbano.dameCiudades(req.query) 
        .then (ciudades => {
            console.log('Piden lista de ciudades y devuelvo', ciudades.length,'elementos');
            ponerCabeceraCORS(req, res, false);
            res.json(ciudades);
...
miAPI.options('/civitas/:id', (req, res) => {
